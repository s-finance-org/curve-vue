	...
	methods: {
	    ...
	    async getAvailableAmount() {
	    	if(!this.$route.params.address) {
	    		...
	    		if(['y', 'iearn', 'dfi', 'dusd', 'okuu'].includes(this.currentPool)) {
	    ...
	    async calcAvailable() {
	    	...
	    	if(['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu'].includes(currentContract.currentContract))
		...
		async calculateAmount(cTokens, block, type) {
		    ...
		    for(let i = 0; i < currentContract.N_COINS; i++) {
		            ...
		            let address = currentContract.coins[i]._address
		          	if(['iearn','busd','pax', 'dfi', 'dusd', 'okuu'].includes(currentContract.currentContract)) address = currentContract.underlying_coins[i]._address
		            ...
		            let usd;
		          	...
		          	if(['iearn','busd', 'pax', 'dfi', 'dusd', 'okuu'].includes(currentContract.currentContract)) {
