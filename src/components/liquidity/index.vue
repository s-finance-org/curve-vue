    ...
    <div class="total-bg">
      <b-container class="d-flex py-4 px-md-5">
        <b-navbar-nav class="navbar-tabs flex-row flex-wrap px-md-5">
          <b-nav-item :to="{ name: 'Liquidity', params: { pool: 'pool5usd' } }">5pool</b-nav-item>
                ...
                <div class="row">
                  <div class="col-12 col-lg mb-2 line-right">
                    ...
                    <div class="mt-4">
                      ...
                      <b-form-checkbox v-show = "!['susd','susdv2','tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)" checked v-model='depositc' name="inf-approval" >{{ $t('liquidity.depositWrapped', [currentPoolTokenCoinMark]) }}</b-form-checkbox>
                    </div>
                  </div>
    ...
    -->
                ...
                <div class="row">
                  <div class="col-12 col-lg mb-2 line-right">
                        ...
                        <div class="currentInput d-flex">
                          <span class="coin d-flex align-items-center" @click='handleCheck(i)'>
                            ...
                            <!-- <b-form-radio :id="'to_cur_'+i" name="withdraw_all" :value='i' @click='handleCheck(i)'> -->
                              ...
                              <span v-show="!withdrawc && !['susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)">{{currency | capitalize}}</span>
                              <span v-show="!withdrawc && ['susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)">{{currencies[currency]}}</span>
                            <!-- </b-form-radio> -->
                          </span>
                          <input class="form-control" type="text"
                            :id="'currency_'+i"
                    ...
                    <div class="mt-4">
                      <b-form-checkbox v-model="inf_approval" name="inf-approval">{{ $t('global.infiniteApproval') }}</b-form-checkbox>
                      <b-form-checkbox v-show = "!['susd', 'susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)" v-model='withdrawc' name="inf-approval" >{{ $t('liquidity.withdrawWrapped', [currentPoolTokenCoinMark]) }}</b-form-checkbox>
                    </div>
                  </div>
                ...
                <div class="d-flex mt-4 justify-content-end">
                  ...
                  <text-overlay-loading :show="loadingAction == 1">
                    <b-button size="lg" variant="danger"
                      ...
                      v-show = "['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool) && staked_balance > 0 "
        ...
        <div class="add-liquidity" v-if=false>
                        ...
                        <li v-for='(currency, i) in Object.keys(currencies)'>
                            <label :for="'currency_'+i">
                              <span class='currency_label'>
                                    ...
                                    <span v-show='depositc'>{{currencies[currency]}}
                                      <span v-show="!(currency == 'usdt' && currentPool == 'usdt' || currency == 'pax') 
                                              && !['susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)"> 
                                        (in {{currency | capitalize}}) 
                                      </span>
                                    </span>
                ...
                <ul>
                    ...
                    <li v-show = "!['susd','susdv2','tbtc','ren','sbtc', 'okuu', 'pool5usd'].includes(currentPool)">
                ...
                <p style="text-align: center" class='buttons'>
                  ...
                  isZeroSlippage: {{ isZeroSlippage }} | 
                    ...
                    <button 
                        id='add-liquidity-stake' 
                        v-show="['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool) && hasRewards" 
                        :disabled = 'slippage < -0.03 || depositingZeroWarning || isZeroSlippage'
                        @click = 'justDeposit = false; deposit_stake()'>
                        Deposit and stake <span class='loading line' v-show='loadingAction == 2'></span>
                    </button>
                    <button id='stakeunstaked' 
                        v-show="totalShare > 0 && ['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool) && hasRewards"
                    ...
                    <div>
                        ...
                        <div v-show='showadvancedoptions'>
                            <fieldset>
                                ...
                                <div v-show="hasRewards && totalShare > 0 && ['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool)">
                                    ...
                                    <button id='stakeunstaked' 
                                        v-show="totalShare > 0 && ['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool)"
                                        :disabled='stakePercentageInvalid' 
                                        @click='stakeTokens()'
                                    >
                                        Stake unstaked <span class='loading line' v-show='loadingAction == 3'></span>
        ...
        <div class="add-liquidity" v-if=false>
            <fieldset class="percentage">
                ...
                <label for='showstaked' v-show="['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool)"> Show staked </label>
                </legend>
                <ul>
                    <li>
                        <input type="text" 
                    ...
                    <li v-for='(currency, i) in Object.keys(currencies)'>
                        <label :for="'currency_'+i" class='currency_label'>
                            <img 
                                ...
                                :src='getTokenIcon(currency, withdrawc)'>
                          <span v-show='withdrawc'>{{currencies[currency]}}
                            <span v-show="!(currency == 'usdt' && currentPool == 'usdt') && !['susdv2', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)">(in {{currency | capitalize}})</span>
                          </span>
                          <span v-show="!withdrawc && !['susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)">{{currency | capitalize}}</span>
                          <span v-show="!withdrawc && ['susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)">{{currencies[currency]}}</span>
                        </label>
            ...
            <fieldset v-show = "currentPool != 'susd'">
              ...
              <ul>
                <li v-show = "!['susdv2','tbtc','ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool)">
            ...
            <div id='withdraw_buttons' class='buttons'>
                ...
                <button id="remove-liquidity"
                    :disabled="['susdv2', 'sbtc', 'okuu', 'pool5usd'].includes(currentPool) && slippage < -0.03 && !warninglow || show_nobalance == true"
                ...
                <button id='claim-snx'
                    ...
                    v-show="['susdv2', 'sbtc','y','iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool) && pendingSNXRewards > 0"
                >
                    Claim {{(pendingSNXRewards / 1e18).toFixed(2)}} {{ ['y','iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool) ? 'YFI' : 'SNX' }}
                ...
                <div id='mintr' v-show="['susdv2', 'sbtc'].includes(currentPool)">
                    ...
                    <a href = 'https://ygov.finance/' v-show="['y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool)" target='_blank' rel="noopener noreferrer"> yGov. </a>
                </div>
            ...
            <div v-show="staked_balance > 0 && ['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool)">
                <button class='simplebutton advancedoptions' @click='showadvancedoptions = !showadvancedoptions'>
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <button id='unstakestaked' 
                                    v-show="staked_balance > 0 && ['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(currentPool)"
...
// maxSlippage: 0.2,
      ...
      computed: {
        ...
        isPlain() {
          return ['susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(this.currentPool)
        },
        transferableBalanceText() {
      ...
      mounted() {
        // withdraw
        if(['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool)) {
        ...
        methods: {
          ...
          async mounted(oldContract) {
            ...
            if(['susd', 'susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(currentContract.currentContract)) this.depositc = true;
            ...
            currentContract.slippage = 0;
              ...
              if(['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool) && +decoded[decoded.length-1] < now)
              ...
              // withdraw
              if(['susdv2', 'tbtc', 'ren', 'sbtc', 'okuu', 'pool5usd'].includes(this.currentPool)) {
                this.withdrawc = true;
            ...
            currentContract.slippage = 0;
              ...
              if(allowance.lte(currentContract.max_allowance.div(BN(2))))
                  this.inf_approval = false
              if(['susdv2', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool)) {
          ...
          async handle_add_liquidity(stake = false) {
            ...
            if(stake && ['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool)) {
      ...
      handleCheck(idx) {
        if(idx === this.to_currency) {
          ...
          async update_balances() {
            ...
            if(['susdv2', 'sbtc','y','iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool)) calls.push([currentContract.curveRewards._address, currentContract.curveRewards.methods.balanceOf(currentContract.default_account || '0x0000000000000000000000000000000000000000').encodeABI()])
          calls.push([currentContract.swap_token._address ,currentContract.swap_token.methods.totalSupply().encodeABI()])
          let aggcalls = await currentContract.multicall.methods.aggregate(calls).call()
          let decoded = aggcalls[1].map(hex => currentContract.web3.eth.abi.decodeParameter('uint256', hex))
          if(currentContract.default_account) {
          ...
          decoded.slice(0, currentContract.N_COINS+1 + currentContract.N_COINS).map((v, i) => {
            Vue.set(this.balances, i, +v)
                if(!currentContract.default_account) Vue.set(this.balances, i, 0)
                  ...
                  if(['susdv2', 'sbtc','y','iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool)) this.staked_balance = BN(decoded[decoded.length-2])
      ...
      setLoadingAction(val) {
        ...
        this.amounts = this.amounts.map(amount => amount || 0)
          ...
          console.log('amounts', this.amounts)
            ...
            if((this.token_balance.lt(BN(token_amount)) || unstake) && ['susdv2', 'sbtc','y','iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool)) {
...
console.log('share', this.share, balance)
            if((this.token_balance.lt(amount) || unstake) && ['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'okuu', 'pool5usd'].includes(this.currentPool)) {
      ...
      console.log('amount', amount.toString(), amount.toFixed(0,1))
            ...
            if(this.to_currency !== null && this.to_currency < 10) {
              ...
              if(!['tbtc','ren','sbtc', 'okuu', 'pool5usd'].includes(currentContract.currentContract)) args.push(this.donate_dust)
              await helpers.setTimeoutPromise(100)
              try {
