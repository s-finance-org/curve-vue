        ...
        <b-tab :title="$t('dao.standTitle')" class="pt-3" v-if=false>
          ...
          <div class="box mb-4 px-4 py-3">
            <div class="row mb-3 line-bottom">
              <span class="col-12 col-md-6 pb-3">
                <h6 class="mb-0 text-black-65">{{ $t('dao.totalStaking') }}</h6>
                <text-overlay-loading inline :show="store.gauges.okuu.mortgages.okuu.totalStaking.loading">
                  <span class="h4 mr-2">{{ store.gauges.okuu.mortgages.okuu.totalStaking.cont }}</span>
                  <span class="inline-block text-black-65">{{ store.gauges.okuu.mortgagesUnit }}</span>
                </text-overlay-loading>
              </span>
              <span class="col-12 col-md-6 pb-3">
                <h6 class="mb-0 text-black-65">{{ $t('dao.myStaking') }}</h6>
                <text-overlay-loading inline :show="store.gauges.okuu.mortgages.okuu.userStaking.loading">
                  <span class="h4 mr-2">{{ store.gauges.okuu.mortgages.okuu.userStaking.cont }}</span>
                  <span class="inline-block text-black-65">{{ store.gauges.okuu.mortgagesUnit }}</span>
                </text-overlay-loading>
              </span>
              <span class="col-12 col-md-6 pb-3">
                <h6 class="mb-0 text-black-65">{{ $t('dao.virtualPrice') }}</h6>
                <text-overlay-loading inline :show="store.tokens.okuu.price.loading">
                  <span class="h4 mb-0">
                    1 <span class="h6 text-black-65">{{ store.tokens.okuu.name }} = </span>
                  </span>
                  <span class="h4 mb-0">
            ...
            <b-tabs pills nav-class="tabs-nav" class="mt-1">
              <b-tab :title="$t('dao.staking')" class="pt-3" active>
                ...
                <div class="row flex-wrap">
                  <div class="col-12 col-lg mt-2">
                    <b-form-input class="h-38" v-model="store.gauges.okuu.mortgages.okuu.stakeAmountInput" :placeholder="$t('dao.stakingAmountPlaceholder')"></b-form-input>
                  </div>
                  <b-form-radio-group
                    class="mt-2 col"
                    v-model="store.gauges.okuu.mortgages.okuu.stakeSliderSelectedRadio"
                    :options="store.gauges.okuu.mortgages.okuu.stakeSliderOptions"
                    buttons
                    button-variant="outline-secondary"
                  ></b-form-radio-group>
                </div>
                <small class="d-flex mt-1 flex-wrap">
                  {{ $t('dao.stakingBalance') }}：
                  <text-overlay-loading class="mr-2" :show="store.gauges.okuu.mortgages.okuu.userBalanceOf.loading">{{ store.gauges.okuu.mortgages.okuu.userBalanceOf.cont }} {{ store.gauges.okuu.mortgages.okuu.name }}</text-overlay-loading>
                  <b-button class="text-blue-1" to='/liquidity/okuu' size="xsm" variant="light">{{ $t('dao.stakingConfirmTip', [store.gauges.okuu.mortgages.okuu.name]) }}</b-button>
                </small>
                <!-- FIXME: inf_approval -->
                <b-form-checkbox class="mt-4" v-model="inf_approval" name="inf-approval">{{ $t('global.infiniteApproval') }}</b-form-checkbox>
                <b-alert class="mt-3" :show="dismissCountDown" variant="dark" dismissible fade
                ...
                <b-alert class="mt-3" :show="store.tokens.okuu.error.dismissCountDown" variant="dark" dismissible fade
                  @dismissed="store.tokens.okuu.error.dismissCountDown=0"
                  v-html='store.tokens.okuu.error.message'>
                </b-alert>

                <div class="d-flex align-items-end mt-5 float-right">
                  <text-overlay-loading :show="loadingAction">
                    <b-button size="lg" variant="danger" @click=onOkuuStake>
                      {{ $t('dao.stakingConfirm') }}
                    </b-button>
                  </text-overlay-loading>
                </div>
              ...
              <b-tab :title="$t('dao.redemption')" class="pt-3">
                ...
                <div class="row flex-wrap">
                  <div class="col-12 col-lg mt-2">
                    <b-form-input class="h-38" v-model="store.gauges.okuu.mortgages.okuu.redemptionAmountInput" :placeholder="$t('dao.redemptionAmountPlaceholder')"></b-form-input>
                  </div>
                  <b-form-radio-group
                    class="mt-2 col"
                    v-model="store.gauges.okuu.mortgages.okuu.redemptionSliderSelectedRadio"
                    :options="store.gauges.okuu.mortgages.okuu.redemptionSliderOptions"
                    buttons
                    button-variant="outline-secondary"
                  ></b-form-radio-group>
                </div>
                <small class="d-flex mt-1">
                  {{ $t('dao.redemptionBalance') }}：
                  <text-overlay-loading :show="store.gauges.okuu.mortgages.okuu.userStaking.loading">{{ store.gauges.okuu.mortgages.okuu.userStaking.cont }} {{ store.gauges.okuu.mortgages.okuu.name }}</text-overlay-loading>
                </small>
                <!-- FIXME: inf_approval -->
                <b-form-checkbox class="mt-4" v-model="inf_approval" name="inf-approval">{{ $t('global.infiniteApproval') }}</b-form-checkbox>
                <b-alert class="mt-3" :show="dismissCountDown && waitingMessageTargetId === 'withdraw'" variant="dark" dismissible fade
                ...
                <div class="d-flex align-items-end mt-5 float-right">
                  <text-overlay-loading :show="loadingAction">
                    <b-button size="lg" variant="danger" @click=onOkuuRedemption>
                      {{ $t('dao.redemptionConfirm') }}
                    </b-button>
                  </text-overlay-loading>
                </div>
              ...
              <b-tab :title="$t('dao.miningReward')" class="pt-3">
                <div class="area">
                  <h5 class="mb-3 d-flex align-items-center">
                    <img :src="getTokenIcon(store.gauges.okuu.rewards.sfg.code)" class="mr-2 icon-w-20 icon token-icon" :class="[store.gauges.okuu.rewards.sfg.code+'-icon']">
                    {{ store.gauges.okuu.rewards.sfg.name }}
                  </h5>
                  <h6 class="mb-0 text-black-65">{{ $t('dao.miningPendingReward') }}</h6>
                  <h4 class="mb-1">
                    <text-overlay-loading inline :show="store.gauges.okuu.rewards.sfg.userPendingReward.loading">
                      {{ store.gauges.okuu.rewards.sfg.userPendingReward.cont }} {{ store.gauges.okuu.rewards.sfg.name }}
                    </text-overlay-loading>
                  </h4>
                  <div class="d-flex no-gutters align-items-end">
                    <small class="col row flex-wrap">
                      <span class="col-12 col-lg-auto">
                        {{ $t('dao.miningPaidReward') }}：
                        <text-overlay-loading inline :show="store.gauges.okuu.rewards.sfg.userPaidReward.loading">
                          {{ store.gauges.okuu.rewards.sfg.userPaidReward.cont }} {{ store.gauges.okuu.rewards.sfg.name }}
                        </text-overlay-loading>
                        <em class="px-3 text-black-15">/</em>
                      </span>
                      <span class="col-12 col-lg-auto">
                        {{ $t('dao.miningTotalReward') }}：
                        <text-overlay-loading inline :show="store.gauges.okuu.rewards.sfg.userTotalReward.loading">
                          {{ store.gauges.okuu.rewards.sfg.userTotalReward.cont }} {{ store.gauges.okuu.rewards.sfg.name }}
                        </text-overlay-loading>
                        <em class="px-3 text-black-15">/</em>
                      </span>
                      <text-overlay-loading class="col-12 col-lg-auto"  inline :show="store.tokens.sfg.price.loading">
                    ...
                    <text-overlay-loading :show="loadingAction">
                      <b-button variant="danger" @click="onOkuuHarvest">
                        {{ $t('dao.miningClaimConfirm') }}
                      </b-button>
                    </text-overlay-loading>
                  </div>
...
<script>
    ...
    import DaoLiquidityGaugereAbi_susdv2 from '../dao/abi/susdv2'
    import DaoLiquidityGaugereAbi_snx from '../dao/abi/snx'
