	...
	<div>
		...
		<div class="window white" v-for='(currency, i) in Object.keys(filteredCurrencies)'>
		      ...
		      <balances-info
			      ...
			      :staked_info = "['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'basu', 'usd5', 'qusd5', 'usdg5', 'busd5'].includes(currency) && staked_infos[currency]"
		...
		methods: {
			...
			async init_contracts() {
			    for(let [key, contract] of Object.entries(this.allContracts)) {
				    ...
				    for (let i = 0; i < contract.N_COINS; i++) {
				        ...
				        let cabi = ['iearn', 'y', 'busd', 'susd', 'pax', 'dfi', 'dusd', 'basu', 'usd5', 'qusd5', 'usdg5', 'busd5'].includes(key) ? yERC20_abi : cERC20_abi;
				        if(key == 'susd' && i == 1) {
				        ...
				        */
			         	if (contract.tethered && contract.tethered[i] 
			            	...
			            	this.all_c_rates[key].c_rates[i] = 1 / contract.coin_precisions[i]
			         		...
			         		if(key == 'iearn' || key == 'y' || key == 'busd' || key == 'dfi' || key == 'dusd' || key == 'basu' || key == 'usd5' || key == 'qusd5' || key == 'usdg5' || key == 'busd5' || (key == 'susd' && i == 0) || (key == 'pax' && i < 3)) {
			            		calls.push([
			...
			async update_fee_info(version = 'new') {
				...
				for(let [key, contract] of Object.entries(this.allContracts)) {
				    ...
				    if(key == 'iearn' || key == 'busd' || key == 'susd' || key == 'susdnew' || key == 'dfi' || key == 'dusd' || key == 'basu' || key == 'usd5' || key == 'qusd5' || key == 'usdg5' || key == 'busd5') {
	            	...
	            	if(['susdv2', 'sbtc', 'y', 'iearn', 'dfi', 'dusd', 'basu', 'usd5', 'qusd5', 'usdg5', 'busd5'].includes(key)) {
	            		let stakedBalance = curveStakedBalance
	            		if(key == 'sbtc') stakedBalance = sbtcCurveStakedBalance
	            		if(['y', 'iearn', 'dfi', 'dusd', 'basu', 'usd5', 'qusd5', 'usdg5', 'busd5'].includes(key)) stakedBalance = yCurveStakedBalance
